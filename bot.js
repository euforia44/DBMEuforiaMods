  ...
  if (names.length === 2) {
    ...
    if (
      baseCommand.options.find(
        (d) => d.name === data.name && d.type === "SUB_COMMAND_GROUP"
      )
    ) {
...
Bot.onReady = function () {
  ...
  let output;
  try {
    betterConsole = require("better-console-s");
    output = betterConsole(`${this.bot.user.username}`, {
    ...
    output = `Bot [${this.bot.user.username}] is Ready!`;
  }
  console.log(output);
  ...
  const fs = require("fs"); // update settings.json
  const path = require("path");
  const settings = require("./data/settings.json");
  const settingsPath = path.join(__dirname, "data", "settings.json");
  const guildIds = this.bot.guilds.cache.map((g) => g.id);
  const botName = this.bot.user.username;
  const avatarHash = this.bot.user.avatar;
